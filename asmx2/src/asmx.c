...
// Patched by Jean-Matthieu COULON from version 2.0b5
...
#define ENABLE_FUNC   // uncomment to enable FUNC pseudo-op (under heavy development, non-standard)

#ifndef VERSION // should be defined on the command line
#define VERSION "2.0"
#endif
#define COPYRIGHT "Copyright 1998-2007 Bruce Tomlin (patched by Jean-Matthieu COULON)"
...
struct FuncRec
{
    struct FuncRec      *next;      // pointer to next function
    Str255              expr;       // function expression
    char                name[1];    // function name
} *funcTab = NULL;
typedef struct FuncRec *FuncPtr;
...
bool            cl_ListSym;         // TRUE to show symbol table in first assembler pass and exit
...
Str255          preInclude[MAX_INCLUDE];    // pre-include file names
Str255          incprefix[MAX_INCLUDE];     // include prefixes
...
{
    printf("*** FROB typ=%d, parm=%d, macro=%.8X, p=%.8X\n",*typ,*parm,(unsigned int)(unsigned long) macro,(unsigned int)(unsigned long) p);
}
...
#endif
        case ADDR_32:
            sprintf(s, "%.8lX ", p->value & 0xFFFFFFFF);
            break;
    }
...
void DefFunc(char *symName)
{
    FuncPtr p;

    if (!(p = FindFunc(symName)))
    {
        // Allocate new function
        p = AddFunc(symName);
    }

    strcpy(p -> expr, linePtr); // store the end of the line as expression
}
...
#ifdef ENABLE_FUNC
bool ExpandFunc(char *symName, int *val)
{
    FuncPtr p;
    int param;
    u_long pval;
    ...
    if (*linePtr == '(' && (p = FindFunc(symName)))
    {
        // Parse parameter values
        param = 1;
        do
        {
            ++linePtr; // Skip value separator
            SkipBlanks();
            if(*linePtr == ')') // End of parameters
            {
              break;
            }

            pval = Eval0(); // Eval parameter
            sprintf(s, "%s.%d", symName, param); // Set value as local symbol
            DefSym(s, pval, FALSE, FALSE);
            ++param;

            SkipBlanks(); // Put linePtr on next separator
        } while (*linePtr == ','); // Continue until no parameter separator

        RParen();

        // Switch subrLabl so Eval0 reads parameter names instead of calling environment
        strcpy(oldSubrLabl, subrLabl);
        strcpy(subrLabl, symName);
        ...
        *val = Eval0();
        ...
        // Switch back to main code flow
        linePtr = oldLine;
        strcpy(subrLabl, oldSubrLabl);
        while (--param > 0)
        {
            sprintf(s, "%s.%d", symName, param); // Set value as local symbol
            UndefSym(s);
...
void write_srec(u_long addr, u_char *buf, u_long len, int rectype)
{
    ...
    {
        case 37:
            fprintf(object,"S%d%.2lX%.8lX", i, len+5, addr & 0xFFFFFFFF);
...
int OpenInclude(char *fname)
{
    ...
    if (include[nInclude])
        return 1;
    for(i = 0; i < MAX_INCLUDE || incprefix[i][0] == '\0'; ++i)
    {
      strcpy(incname[nInclude],incprefix[i]);
      strcpy(incname[nInclude] + strlen(incprefix[i]),fname);
      include[nInclude] = fopen(incname[nInclude], "r");
      if (include[nInclude])
          return 1;
    }
...
//  fprintf(stderr, "    -9                  output object file in Motorola S9 format (16-bit address)\n");
    ...
    fprintf(stderr, "    -i filename         pre-include a file on top of source\n");
    fprintf(stderr, "    -I prefix           add an include prefix (must be / terminated for directories)\n");
...
void getopts(int argc, char * const argv[])
{
    ...
    // Reset include prefixes
    incprefix[0][0] = '\0';

    while ((ch = getopt(argc, argv, "ew19tb:cd:l:o:s:C:i:I:?")) != -1)
    {
        errFlag = FALSE;
        switch (ch)
        {
            ...
            case 'i':
                strncpy(preInclude[preinc], optarg, 255);
                ++preinc;
                break;

            case 'I':
                strncpy(incprefix[curincprefix], optarg, 255);
                if (incprefix[curincprefix][0] && incprefix[curincprefix][strlen(incprefix[curincprefix]) - 1] != '/')
                {
                    strncat(incprefix[curincprefix], "/", 255);
                }
                ++curincprefix;
                break;
