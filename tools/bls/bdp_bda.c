...
struct bdppriv {
  ...
  u8 inp[36];
  int inpl;
  u8 cmdbuf[CMDBUFMAX][36];
  int cmdbufl;
  int bdpdump;
  int cpustate[3];
  int busstate[3];
  u32 regaddr[3];
  unknown_handler_ptr on_unknown;
  breakpoint_handler_ptr on_breakpoint;
  exception_handler_ptr on_exception;
  u32 ga_status;
  u32 breakpoints[2][MAX_BREAKPOINTS + 1][2];
  void (*waitack)();
}
