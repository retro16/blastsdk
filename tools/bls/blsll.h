...
#define BLSLL_DECLARE(name, freedata) BLSLL(name) { name *data; BLSLL(name) *next; }; \
  ...
  static inline BLSLL(name) * blsll_remove_##name(BLSLL(name) *list) { \
    if(!list->next) { free(list); return NULL; } \
    list->data = list->next->data; BLSLL(name) *n = list->next->next; free(list->next); list->next = n; return list; } \
  static inline BLSLL(name) * blsll_removedata_##name(BLSLL(name) *list) { \
    if(list->data) { freedata((list)->data); } return blsll_remove_##name(list); } \
