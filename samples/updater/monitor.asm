...
monitor_init	; Call this to initialize the monitor
		bsr.w	bdp_init		; Initialize the debugger

		lea	VDPDATA, a5
		lea	VDPCTRL, a6
		VDPUSEREG			; Use a5/a6 for VDP macros
		...
		VDPDMASEND FONT_PNG_CONCAT, FONT_PNG, FONT_PNG_SIZE/2, VRAM ; Upload font tiles
		endif

		VDPSETWRITE 0, CRAM
		move.l	#$00000AAA, (a5)	; Set background to black and text to grey
		VDPSETWRITE 34, CRAM		; Point at palette 2, index 1
		move.w	#$00E0, (a5)		; Set highlighted text color

		VDPSETWRITE HSCROLL, VRAM
		move.l	#$0, (a5)		; Reset horizontal scrolling
		VDPSETWRITE 0, VSRAM
		move.l	#$0, (a5)		; Reset vertical scrolling
...
printline
		...
		move.w	(a0, d7), (a5)	; Print mode character
...
interrupt
		VDPSETBG $608			; Magenta background = unknown interrupt
		rte
